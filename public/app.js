...
function renderHand() {
  ...
  if (STATE.youFinished) {
    hand.innerHTML = `<div class="finished-msg">🎉 You finished ${ordinal(STATE.yourPlacement)}! Watching the rest of the round…</div>`;
    $('.hand-actions').style.display = 'none'; $('#you-tag').textContent = ''; return;
  }
...
function renderScore() {
  ...
  // Round results order by placement; the game-over screen orders by total score.
  const ranked = over
    ? [...STATE.players].sort((a, b) => b.score - a.score)
    : [...STATE.players].sort((a, b) => (a.placement || 99) - (b.placement || 99));
  const winner = STATE.players.find((p) => p.id === STATE.winnerId);
  const champ = over ? ranked[0] : winner;
  $('#winner-badge').innerHTML = champ
    ? `<span class="crown">👑</span>${esc(champ.name)} ${over ? 'wins the game!' : 'finished 1st!'}`
    : '';
  const body = $('#score-body'); body.innerHTML = '';
  ranked.forEach((p, i) => {
    const tr = document.createElement('tr');
    if (p.placement === 1) tr.className = 'win';
    const place = p.placement ? medal(p.placement) : (i + 1);
    const gained = p.lastRoundPoints ? '+' + p.lastRoundPoints : '';
    tr.innerHTML = `<td>${over ? (i + 1) : place}</td><td>${p.avatar} ${esc(p.name)}</td>
      <td>${gained}</td><td>${p.score}</td><td>${p.wins}</td>`;
    body.appendChild(tr);
  });
...
function handleAction(s) {
  ...
  const map = {
    ...
    finish: () => announce(`${byName} finished ${ordinal(a.placement)}! 🎉`, 'var(--accent2)'),
    roundOver: () => { announce('🏁 Round over!', 'var(--accent2)'); Sound.play((STATE.players.find(p=>p.id===s.you)?.placement===1) ? 'win' : 'lose'); }
  };
  (map[a.type] || (() => {}))();
}
...
function addChat(m) {
  ...
  // Float the message into the game so nobody has to open the panel.
  floatChat(m);
}

// A live chat feed that drifts onto the table and auto-dismisses.
function ensureChatFlow() {
  let flow = document.getElementById('chat-flow');
  if (!flow) {
    flow = document.createElement('div');
    flow.id = 'chat-flow';
    flow.className = 'chat-flow';
    $('#screen-game').appendChild(flow);
  }
  return flow;
}
function floatChat(m) {
  if (!isGameScreen()) return;          // only overlay during play
  const flow = ensureChatFlow();
  const b = document.createElement('div');
  b.className = 'flow-bubble' + (m.system ? ' sys' : '');
  b.innerHTML = m.system
    ? esc(m.text)
    : `<b>${m.avatar || ''} ${esc(m.name)}</b> ${esc(m.text)}`;
  flow.appendChild(b);
  while (flow.children.length > 6) flow.removeChild(flow.firstChild);
  setTimeout(() => { b.classList.add('leaving'); setTimeout(() => b.remove(), 500); }, 5200);
}

// ===================== EMOJI FLOAT =====================
function floatEmoji({ emoji }) {
  const layer = $('#emoji-layer');
  const count = 10 + Math.floor(Math.random() * 6);   // a burst, not just one
  for (let i = 0; i < count; i++) {
    ...
    e.style.bottom = (10 + Math.random() * 26) + '%';
    e.style.animationDuration = (1.7 + Math.random() * 1.3) + 's';
...
function ordinal(n) { n = n || 0; const s = ['th','st','nd','rd'], v = n % 100; return n + (s[(v - 20) % 10] || s[v] || s[0]); }
