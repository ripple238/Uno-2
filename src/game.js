...
class UnoGame {
  ...
  resolveCard(player, card, afterColor = false) {
    const justFinished = (player.hand.length === 0);

    // 7-0 house rule (skipped when the player just went out).
    if (this.rules.sevenZero && !isWild(card) && !justFinished) {
    ...
    if (justFinished) return this.handleFinish(player, card);
  ...
  /** Record a player going out: assign placement, award points, leave the turn order. */
  recordPlacement(player) {
    player.finished = true;
    player.placement = ++this.finishCount;
    let pts = 0;                                  // points = cards still held by everyone still in
    for (const p of this.players) if (!p.finished) pts += p.hand.reduce((s, c) => s + cardPoints(c), 0);
    player.score += pts;
    player.lastRoundPoints = pts;
    if (player.placement === 1) { player.wins += 1; this.winnerId = player.id; }
    this.order = this.order.filter((id) => id !== player.id);
  }

  /** A player just emptied their hand. They sit out and watch; the rest keep playing. */
  handleFinish(player, card) {
    const nextId = this.currentId;                // who plays next (captured before removal)
    this.recordPlacement(player);
    if (this.order.length <= 1) {                 // only one player left -> round is done
      this.endRound(card);
      return { ok: true, roundOver: true, finished: player.id };
    }
    const pos = this.order.indexOf(nextId);
    this.turnPos = pos < 0 ? 0 : pos;
    this.setAction({ type: 'finish', by: player.id, placement: player.placement, card });
    return { ok: true, finished: player.id, placement: player.placement };
  }

  endRound(lastCard) {
    if (this.order.length === 1) {                // the lone survivor places last
      const last = this.player(this.order[0]);
      last.finished = true;
      last.placement = ++this.finishCount;
      last.lastRoundPoints = 0;
      last.losses += 1;
    }
    this.phase = 'roundOver';
    if (!this.winnerId) {
      const first = this.players.find((p) => p.placement === 1);
      if (first) this.winnerId = first.id;
    }
    this.setAction({ type: 'roundOver', by: this.winnerId, card: lastCard });
    if (this.rules.targetScore > 0 && Math.max(...this.players.map((p) => p.score)) >= this.rules.targetScore) {
      this.phase = 'gameOver';
    }
  }
...
class RoomManager {
  ...
  publicState(room, viewerId) {
    ...
    const mapPlayer = (p, seatIndex) => ({
      ...
      calledUno: p.calledUno, finished: !!p.finished, placement: p.placement || 0,
      lastRoundPoints: p.lastRoundPoints || 0, seatIndex
    });
    ...
    if (g) {
      ...
      base.youFinished = !!(me && me.finished);
      base.yourPlacement = me ? (me.placement || 0) : 0;
